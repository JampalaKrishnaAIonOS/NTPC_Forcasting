...
def forecast_lightgbm(plant_key: str, series_df: pd.DataFrame):
    ...
    # Return forecast df with CI columns in required order: conf96__lower, powerGW, conf96__upper
    df_forecast_out = df_forecast.copy()
    df_forecast_out['conf96__lower'] = conf_lower
    df_forecast_out['conf96__upper'] = conf_upper
    df_forecast_out = df_forecast_out[['conf96__lower', 'powerGW', 'conf96__upper']]
    df_forecast_out = df_forecast_out.rename(columns={"powerGW": f"ps_{plant_key}__FORECAST"})
